...
<div class="col-12">
    <a href="{{route('createup')}}"> <button class="btn btn-success"><i class="fas fa-upload"></i> อัพไฟล์เอกสาร</button> </a>
</div>
    ...
    <div class="col-12">
        <div class="card">
            <div class="card-header">
                <h3 class="card-title"> <strong>ตรวจเอกสารคำร้องขอซ่อมกิจกรรม</strong> </h3>
                <div style="float: right;">
                    <h3 class="card-title"> <strong> จํานวนทั้งหมด {{$statusup->count()}} รายการ</strong> </h3>
                </div>
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td class="text-center">
                                <a href="{{ asset('font_end/upload/'.$status->file)}}" class="" target="_blank">
                                    <div class="btn btn-primary"><i class="fa fa-eye"></i></div>
                                </a>
                                <a href="{{url('admin/status/upload/editup/'.$status->id)}}">
                                    <div class="btn btn-warning"><i class="fa fa-edit"></i></div>
                                </a>
                                <a href="{{url('admin/status/upload/deleteup/'.$status->id)}}">
                                    <div class="btn btn-danger"><i class="fa fa-trash"></i></div>
                                </a>
                                <a href="{{url('admin/status/comment/'.$status->id)}}">
                                    <div class="btn btn-info"><i class="fa fa-comment-dots"></i></div>
                                </a>

                                @if ($status->edit == 0)
                                    <form action="{{ url('/admin/upload/open/'.$status->id) }}" method="post">
                                      @csrf
                                      <button type="submit" class="btn btn-danger" name="edit" value = "1">ปิดปุ่มแก้ไข</button>
                                    </form>
                                @elseif ( $status->edit == 1)
                                      <form action="{{ url('/admin/upload/end/'.$status->id) }}" method="post">
                                        @csrf
                            ...
                            <input type="submit" class="btn btn-success" name="status" value="ดำเนินการแล้ว" />
                                ...
                                <form action="{{url('admin/status/pass/'.$status->id)}}" method="post">
                                    @csrf
